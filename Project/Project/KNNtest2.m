...
end_cand = find(nodes(1,:)>295);
begin_cand = find(nodes(1,:)<5);
for n = 1 : length(begin_cand)
    for m = 1 : length(end_cand)
       [optimalpath,cost] = Djikstras(begin_cand(n),end_cand(m),djikstrasinput);
       optimalpath_store{n,m} = optimalpath;
       cost_store(n,m) = cost;
    end
end
[min_row,min_col] = find(cost_store == min(min(cost_store)));
final_route = optimalpath_store{min_row,min_col};
for n = 1 : length(final_route)
    pathnodes(:,n) = nodes(:,final_route(n));
end
figure
for k = 1 : numObst
     Ellipse_plot(As{k},cs{k});
