...
% Calculate Mean anomaly at time of Herricks Gibbs Reading
mu = 3.986005e14;
n = sqrt(mu/(orbit_params(1,1))^3);
MeanAnomaly = orbit_params(1,6) + n*(time2(gs_num));
k = 1;

% Calculate orbit using real orbital parameters at the time of the Herricks
% Gibbs Reading
...
figure
plot(1:time_period,ECI_est(2,:),'r')
hold on
plot(1:time_period,ECI_sat(2,:),'b')
legend('Estimated Y Position','Real Y Position')
title('ECI Y position vs Time')
xlabel('Time (seconds)')
ylabel('Position (m)')

figure
plot(1:time_period,ECI_est(3,:),'r')
hold on
plot(1:time_period,ECI_sat(3,:),'b')
legend('Estimated Z Position','Real Z Position')
title('ECI Z position vs Time')
xlabel('Time (seconds)')
ylabel('Position (m)')
