...
sigma_star = 8.25/3/3600*deg2rad;
sigma_mag = 0.75*deg2rad/3;        % 1 sigma in radians
...
for t = 1:step:time_period
    ...
    % Convert Body to RAE
    Mag_RAE(t,:) = Cart_to_Polar(Mag_Body(t,:));
    ...
    % Convert RAE with noise back to Body
    Mag_Body(t,:) = Cart_to_Polar(Mag_RAE);
    ...
    for k = 1:r
        ...
        % Convert Body to RAE
        Star_RAE(k,:,t) = Cart_to_Polar(Star_Body(k,:,t));
        
        % Add noise to RAE
        Star_RAE(k,1,t) = Star_RAE(k,1,t);
        Star_RAE(k,2,t) = normrnd(Mag_RAE(t,2), sigma_star);
        Star_RAE(k,3,t) = normrnd(Mag_RAE(t,3), sigma_star);
        
        % Convert RAE with noise back to Body
        Star_Body(k,:,t) = Polar_to_Cart(Star_RAE(k,:,t));
    end
    
    % Progress Report
    if t == 10001 || t==20001 ||t==30001||t==40001||t==50001||t==60001||t==70001||t==80001
        disp('Measurement Generation Timestep = ')
        disp(t)
    end
...
weight_starY = 1;
weight_starP = 1;
weight_starR = 1;    %since error in roll is larger
