...
function output = Orbit_to_ECI_and_Simulate(a,e,i,omega,w,Mo,tsinceMo,timeperiod)
...
for k = 1:timeperiod
    ...
    % Orbital coordinates
    orbit_x(k) = r(k)*cos(theta(k)); %Add intitial starting angle
    orbit_y(k) = r(k)*sin(theta(k)); 
    orbit_z(k) = 0 ;
    orbit_vec(:,k) = [orbit_x(k);orbit_y(k);orbit_z(k)];
    %apply conversion matrix
    P_ECI(:,k) = C*orbit_vec(:,k);
end
%output the ECI
output = P_ECI;
end